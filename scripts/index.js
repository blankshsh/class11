$(document).ready(function() {
    ...
    $(function() { //职业路径图
        ...
        $(".projectpath-inner a").on("mouseenter", function() {
            // 鼠标移上变色，变自己上下左，变化下一个标签的左边框
            $(this).removeClass("on-old").addClass('on-green');
            $(this).next().removeClass('on-old').addClass('on-old2');

        })
        $(".projectpath-inner a").on("mouseleave", function() {
            //鼠标移走，恢复原状。
            $(this).removeClass("on-green").addClass('on-old');
            $(this).next().removeClass('on-old2').addClass('on-old');

        })
        $(".before-right").on("mouseenter", function() {
            //特殊情况，距离右边的标签变化
            $(this).removeClass("on-old").addClass('on-green');
            $(this).next().removeClass('on-right').addClass('on-right2');
        })
        $(".before-right").on("mouseleave", function() {
            //特殊情况，最右第二个标签变化
            $(this).removeClass("on-green").addClass('on-old');
            $(this).next().removeClass('on-right2').addClass('on-right');
        })

        $(".on-right").on("mouseenter", function() {
            //鼠标移动到最右边标签的变化
            $(this).removeClass("on-right").addClass("on-green-right");
        })
        $(".my-on-right").on("mouseleave", function() {
            $(this).removeClass("on-green-right").addClass("on-right");
        })
    })
    $(function() { //知识体系图
